        ...
        if (data.success && data.data?.user) {
          ...
          if (userData.avatar) {
            userData.avatar = userData.avatar.startsWith('http') 
              ? userData.avatar 
              : `${apiUrl}/storage/${userData.avatar}`;
          }
          setUser(userData);
  ...
  useEffect(() => {
    ...
    const checkAuth = async () => {
      ...
      if (token) {
        ...
        // If API call failed, fallback to localStorage
        if (!userData && userStr) {
          try {
            const parsedUser = JSON.parse(userStr);
            // Format avatar URL if it exists and is not already a full URL
            if (parsedUser.avatar && !parsedUser.avatar.startsWith('http')) {
              const apiUrl = process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8000';
              const avatarPath = parsedUser.avatar.replace(/^storage\//, '');
              parsedUser.avatar = `${apiUrl}/storage/${avatarPath}`;
            }
            setUser(parsedUser);
          } catch (error) {
    ...
    <div className="AvatarDropdown ">
      <Popover className="relative">
        {({ open, close }) => (
          <>
            <Popover.Button
              className={`w-10 h-10 sm:w-12 sm:h-12 rounded-full text-slate-700 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800 focus:outline-none flex items-center justify-center overflow-hidden`}
            >
              {user?.avatar ? (
                <Avatar 
                  imgUrl={user.avatar} 
                ...
                <div className="overflow-hidden rounded-3xl shadow-lg ring-1 ring-black ring-opacity-5">
                  <div className="relative grid grid-cols-1 gap-6 bg-white dark:bg-neutral-800 py-7 px-6">
                    <div className="flex items-center space-x-3">
                      <Avatar 
                        imgUrl={user?.avatar || avatarImgs[7]} 
                        sizeClass="w-12 h-12" 
                      />

                      <div className="flex-grow min-w-0">
                        <h4 className="font-semibold truncate">{user?.name || 'User'}</h4>
                        <p className="text-xs mt-0.5 text-neutral-500 dark:text-neutral-400 truncate">{user?.email || ''}</p>
                        {user?.roles && user.roles.length > 0 && (
                          <p className="text-xs mt-0.5 text-primary-6000 dark:text-primary-400">
                            {user.roles[0]}
                          </p>
                        )}
                      </div>
