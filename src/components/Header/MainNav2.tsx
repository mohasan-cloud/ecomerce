...
const MainNav2: FC<MainNav2Props> = ({ className = "" }) => {
  ...
  const renderSearchForm = () => {
    return (
      <div ref={searchFormRef} className="flex-1 py-2 text-slate-900 dark:text-slate-100 relative">
        <form
          onSubmit={(e) => {
            e.preventDefault();
            if (searchQuery.trim()) {
        ...
        >
          <div className="bg-slate-50 dark:bg-slate-800 flex items-center space-x-1.5 px-5 h-full rounded relative">
        ...
        {showDropdown && searchQuery.trim().length >= 2 && (
          <SearchDropdown searchQuery={searchQuery} onClose={() => setShowDropdown(false)} />
        )}
      </div>
    );
  };
